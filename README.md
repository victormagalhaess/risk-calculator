...
This is the implementation of [Origin's Backend Take Home Assignment](https://github.com/OriginFinancial/origin-backend-take-home-assignment) application.
It aims to determine the user’s insurance needs by asking personal & risk-related questions and gathering information about the user’s vehicle and house.
...
Both ways were intended to be run on Ubuntu 20.04 or newer, however it may run without too much problem on any other system that contains the needed software to the execution. Keep in mind that the probably best way (and recommended way) to run the application is using ***Docker on Ubuntu***.
...
After running, the application will be live on the localhost at the port number set on Makefile's variable PORT. You can change it as you please, but the default port is 8080.
...
## Documentation
The project counts with automated documentation for the api, using the [Open Api's Swagger](https://swagger.io/specification/). The documentation is already built, but can be regenerated (using the local Go installation) using the command:
...
This project raised the opportunities to use a lot of knowledge that isn't code, such as:
...
The packages responsible to the flow showed on the chart above are first:
...
- [pipeline](https://github.com/victormagalhaess/origin-backend-take-home-assignment/tree/main/pkg/pipeline/):
    - The pipeline package implements a struct that holds steps. It is a holder that the next package, engine, uses to save the steps that will be executed on the user input.

- [engine](https://github.com/victormagalhaess/origin-backend-take-home-assignment/tree/main/pkg/engine):
    - The engine package is the actual "engine" for the risk calculation. It initializes the pipeline with all the steps that will be used to build the risk profile, then, on receiving a user information object it build a risk profile and call each step to build an risk profile based on the steps results. 

Using these packages is really simple to add or remove steps to the risk algorithm. The engine fills the pipeline with the needed steps, so in order to change it, you just need to write any new steps using the pattern of the other steps and add or remove them when the [engine is fueled](https://github.com/victormagalhaess/origin-backend-take-home-assignment/blob/515ff2c6144b0371ea3c98c277576c72b6f8eebb/pkg/engine/engine.go#L11).
...
The controllers were responsible to deal with the REST interface and operations, were all the business logic and communication with other system packages must be in the services.
